...
description = "AceBot - A multifunction Discord bot by VPCat"
...
class Bot(commands.Bot):
    ...
    async def shutdown(self, *, restart=False):
        """Gracefully quits AceBot with exit code 0

        If restart is True, the exit code will be 26 instead
        The launcher automatically restarts AceBot when that happens"""
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    async def on_ready():
        ...
        print("\nOfficial server: https://discord.gg/GTW6sVJ")
...
def interactive_setup(settings):
    ...
    if first_run:
        print("AceBot - First run configuration\n")
...
def load_cogs(bot):
    ...
    if owner_cog is None:
        print("The owner cog is missing. It contains core functions without "
              "which AceBot cannot function. Reinstall.")
        exit(1)
